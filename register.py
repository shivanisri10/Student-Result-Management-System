...
class Register:
    def __init__(self,root):
        ...
        self.var_chk=IntVar()
        Checkbutton(frame1,text="I Agree The Terms & Conditions",variable=self.var_chk,onvalue=1,offvalue=0,bg="white",font=("times new roman",12)).place(x=50,y=380)

        Button(frame1,text="Register Now",font=("times new roman",15,"bold"),bg="green",fg="white",cursor="hand2",command=self.register_data).place(x=50,y=420,width=250)
        Button(self.root,text="Sign In",font=("times new roman",18,"bold"),bg="white",bd=0,cursor="hand2",command=self.login_window).place(x=170,y=510,width=200)
        ...
        if self.txt_fname.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                if row!=None:
                    ...
                    cur.execute("insert into employee (f_name,l_name,contact,email,question,answer,pass) values(?,?,?,?,?,?,?)",
                                    (self.txt_fname.get(),self.txt_lname.get(),self.txt_contact.get(),self.txt_email.get(),self.cmb_quest.get(),self.txt_answer.get(),self.txt_password.get()))
                    con.commit()
                    con.close()
